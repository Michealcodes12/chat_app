...
export function ChatWindow({ recipientId }: { recipientId: string }) {
  ...
  useEffect(() => {
    ...
    async function setupChat() {
      try {
        ...
        const { public_key: recipientPubKeyBase64 } = await getPublicKey(recipientId);
        ...
        for (const msg of history) {
          try {
            const keyToDecrypt = msg.from_user_id === user!.id 
        ...
        decryptedHistory.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
        ...
        if (isMounted) {
          showToast(e.message || "Failed to setup chat", "error");
          setIsFetching(false);
        }
      }
    }
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputText.trim() || isSending || !recipientKeyRef.current || !selfPublicKeyRef.current) return;
    ...
    try {
      // 1. Encrypt
      const payload = await encryptMessage(textToSend, recipientKeyRef.current, selfPublicKeyRef.current);
      ...
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        ...
        setMessages(prev => prev.map(m => m.id === tempId ? { ...m, id: response.id } : m));
      }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-foreground/50">{isConnected ? 'Connected' : 'Disconnected'}</span>
            </div>
          </div>
        </div>
      </div>
